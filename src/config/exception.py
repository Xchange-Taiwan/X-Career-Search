...
def __client_exception_handler(request: Request, exc: ClientException):
    return JSONResponse(
        ...
        content=res_err_format(msg=exc.msg, code=exc.code, data=exc.data),
    )


def __unauthorized_exception_handler(
...
def __not_acceptable_exception_handler(
    request: Request, exc: NotAcceptableException
):
    return JSONResponse(
        status_code=exc.status_code,
        content=res_err_format(msg=exc.msg, code=exc.code, data=exc.data),
...
def __duplicate_user_exception_handler(
    request: Request, exc: DuplicateUserException
):
    return JSONResponse(
        status_code=exc.status_code,
        content=res_err_format(msg=exc.msg, code=exc.code, data=exc.data),
...
def __too_many_requests_exception_handler(
...
def include_app(app: FastAPI):
    ...
    app.add_exception_handler(
        UnauthorizedException, __unauthorized_exception_handler
    )
    app.add_exception_handler(
    ...
    app.add_exception_handler(
        NotAcceptableException, __not_acceptable_exception_handler
    )
    app.add_exception_handler(
        DuplicateUserException, __duplicate_user_exception_handler
    )
    app.add_exception_handler(
        TooManyRequestsException, __too_many_requests_exception_handler
    )
