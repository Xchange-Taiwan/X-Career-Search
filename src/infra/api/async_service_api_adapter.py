    ...
    def decorator_check_response_code(func):
        ...
        async def wrapper_check_response_code(*args, **kwargs):
            ...
            if status_code is not None and (
                status_code < 400 or status_code == expected_code
            ):
...
class AsyncServiceApiAdapter(IServiceApi):
    ...
    async def get(
        ...
        try:
            ...
            log.error(
                f"simple_get request error, url:%s, params:%s, headers:%s, resp:%s, err:%s",
                url,
                params,
                headers,
                response,
    ...
    async def simple_post(
        self, url: str, json: Dict, headers: Dict = None
    ) -> Optional[Dict[str, Any]]:
    ...
    async def post(
        ...
        try:
            ...
            log.error(
                f"simple_post request error, url:%s, json:%s, headers:%s, resp:%s, err:%s",
                url,
                json,
                headers,
                response,
    ...
    async def put(
        ...
        try:
            ...
            log.error(
                f"simple_put request error, url:%s, json:%s, headers:%s, resp:%s, err:%s",
                url,
                json,
                headers,
                response,
    ...
    async def delete(
        ...
        try:
            ...
            log.error(
                f"simple_delete request error, url:%s, params:%s, headers:%s, resp:%s, err:%s",
                url,
                params,
                headers,
                response,
